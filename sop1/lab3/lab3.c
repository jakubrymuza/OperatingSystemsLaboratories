...
void *overseer(void * args)
{
	...
	struct timespec rqtp = {s,ms*ms2ns};
...
{	
	...
	while(1)
	{
		pthread_mutex_lock(&mxseed);
		int ms = rand_r(&seed)%(SLEEP_MAX-SLEEP_MIN+1)+SLEEP_MIN;
		pthread_mutex_unlock(&mxseed);
		struct timespec rqtp = {0,ms*ms2ns}, rmtp;
